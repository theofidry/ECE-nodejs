...
module.exports = function (globals, path) {
    ...
    db.get('user:admin', function (err, value) {
        ...
        // No error: admin user properly retrieved
        console.log('Database initialised.');
    });

    var read = fs.createReadStream(__dirname + '/users.csv')
        //.pipe(db.createWriteStream())
        .on('data', function(line) {
          //storage of user's data
          var infoUserArray = line.toString().split('\r\n');
          ...
          for(var i=0; i<infoUserArray.length-1; i++){
            var user = infoUserArray[i].split(',');
            var keyuser = 'user:'+ user[0];
            var valueuser = user[2];
            var keyusermail= 'user:' + user[1];
            dest.write({key: keyuser , value: valueuser});
            dest.write({key: keyusermail , value: user[0]});
          }
          dest.end();
        })
       .on('close', function(){
            console.log('db close: initialization completed');
        })
